            ...
            <div class="card-body">
                ...
                @endif
                    ...
                    <div class="row">
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label for="Course_id">{{trans('Students_trans.Course')}} : <span class="text-danger">*</span></label>
                                    <select class="custom-select mr-sm-2" name="Course_id">
                                        <option selected disabled>{{trans('Parent_trans.Choose')}}...</option>
                                        @foreach($Courses as $Course)
                                            <option value="{{ $Course->id }}" {{$Course->id == $Students->Course_id ? 'selected' : ""}}>{{ $Course->Name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function () {
            $('select[name="Course_id"]').on('change', function () {
                ...
                if (Course_id) {
                    $.ajax({
                        url: "{{ URL::to('Get_classrooms') }}/" + Course_id,
                        type: "GET",
                        dataType: "json",
